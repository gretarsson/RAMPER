...
def FVA(model, settings, fileName='', fluxes=-1, inpFvaSettings=None, epsPerc=0.01, lowerBound=10 ** -8, upperBound=10 ** 3, nOfProcessors=cpu_count() / 2):
    ...
    if __name__ == '__main__':
        ...
        try:
            arguments = ', epsPerc = '+str(epsPerc)+', lowerBound = '+str(lowerBound)+', upperBound = '+str(upperBound)
        ...
        if fileName != '':
            mesg(settings['logFile'], '\nFVA results saved as ' + fileName + ' using the pickle package.')
...
fvaData = FVA('ecYeast_batch.mat', settings, filName='./Results/test', inpFvaSettings=fvaSettings, fluxes=range(10))