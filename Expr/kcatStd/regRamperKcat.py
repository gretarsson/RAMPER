...
def findMaxStd(kcatIndecesList, uncMat, grwthCutoff, tnCons, maxIter, protRows, i):
	...
	uncMat[rowInd][kcatIndeces] = tnCons[0]*uncMat[rowInd][kcatIndeces]
	...
	while tnGrwthRate > grwthCutoff and counter <= maxIter:
		uncMat[rowInd][kcatIndeces] = tnCons[counter-1]*orRow[rowInd][kcatIndeces]
		try:
			settings = {'uncertaintyMatrix': uncMat,
				    'print': False}
			RAMPERpars, stat = createUncModel(model, settings)
...
def main():
	...
	maxIter = 11  # max iteration of increase in enzyme's kcats before we stop
	tnCons = [10**(i+1) for i in range(maxIter)] 
	#tnCons = np.linspace(10**2, 3.03*10**5, maxIter)
	# testing some values first
	#tnCons = []
